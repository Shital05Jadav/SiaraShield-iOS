...
public extension ProgressHUD {
    ...
    class func show(_ status: String? = nil, icon: AlertIcon, interaction: Bool = true, delay: TimeInterval? = nil) {
      ...
      //  let image = icon.image//withTintColor(shared.colorAnimation, renderingMode: .alwaysOriginal)
        ...
        DispatchQueue.main.async {
            shared.setup(status: status, staticImage: UIImage(), hide: true, interaction: interaction, delay: delay)
        }
    }
...
public class ProgressHUD: UIView {
    ...
    private var colorStatus        = UIColor.black
    ...
    private var imageSuccess    = UIImage()
        //.checkmark.withTintColor(UIColor.systemGreen, renderingMode: .alwaysOriginal)
    private var imageError        = UIImage()
        //.remove.withTintColor(UIColor.systemRed, renderingMode: .alwaysOriginal)
